...
public final class Battle extends BaseBattle {
    ...
    private Map<RadarType, RadarTypePeer> RadarValues;
    private Map<CannonType, CannonTypePeer> CannonValues;
    private Map<ShipType, ShipTypePeer> ShipValues;
    ...
    void setup(RobotSpecification[] battlingRobotsList, BattleProperties battleProps, boolean paused) {
        ...
        battleRules = HiddenAccess.createRules(battleProps.getBattlefieldWidth(), battleProps.getBattlefieldHeight(),
                ...
                battleProps.getHideEnemyNames(), battleProps.getSentryBorderSize(), battleProps.getRadarConfiguration(), battleProps.getCannonConfiguration(), battleProps.getShipConfiguration(), battleProps.getSingleBarrelBulletRange(), battleProps.getDoubleBarrelBulletRange());

        createRadarPeers(battleRules.getRadarConfig());
        createCannonPeers(battleRules.getCannonConfig());
        createShipTypePeers(battleRules.getShipConfig());
...
//        if (battleRules.isCustomConfig()) {
            ...
            for (int i = 0; i < parts.length; i++) {
                ...
                RadarValues.put(type, new RadarTypePeer(
                        ((battleRules.getBattlefieldHeight() / 100) * Integer.parseInt(parts[i].substring(parts[i].indexOf('[') + 1, parts[i].indexOf('-')))),
...
//        }
    ...
    public void addMissile(MissilePeer missile) {
    ...
    private void loadRobotAttributes(){
        if(HiddenAccess.getNaval()) {
            for (RobotPeer robotPeer : robots) {
                ShipPeer peer = (ShipPeer) robotPeer;
                peer.updateAttributes();
            }
        }
    }
    ...
    protected void preloadRound() {
        ...
        if (getRoundNum() == 0) {
            ...
            if (isPaused) {
                //TODO this may cause the pause bug
    ...
    private void setRobotComponentAttributes(){
        if (HiddenAccess.getNaval()) {
            for (RobotPeer robotPeer : robots) {
                ShipPeer peer = (ShipPeer) robotPeer;
                peer.updateComponents();
            }
        }
    }
    ...
    protected void runRound() {
        //todo set alt values
        setRobotComponentAttributes();
//        System.out.println("Run round at: " + System.currentTimeMillis());
