...
//		type = component.getType();
		...
		slotIndex = component.getSlotIndex();
	 ...
	 */
	protected BaseComponentSnapshot(Point2D.Double pivot, int color, double angle,
//									ComponentType type,
									double lastHeading, byte serializeType){
		this.pivot = pivot;
		this.color = color;
		this.angle = angle;
//		this.type = type;
...
//			writer.writeAttribute("type", type.toInt(), options.trimPrecision);
		...
		writer.writeAttribute("slotindex", slotIndex, options.trimPrecision);
//		writer.endElement();
