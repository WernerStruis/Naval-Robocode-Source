    ...
    public void update(List<RobotPeer> robots, List<MissilePeer> missiles, List<BulletPeer> bullets) {
        ...
        if (isActive()) {
            if (HiddenAccess.getNaval()) {
                if(getState() == MissileState.ARRIVED) {
        ...
        if (ships !=null) {
            for (RobotPeer otherShip : ships) {
                if (!(otherShip == null || otherShip.isDead())) {
                    ...
                    ShipType type = ((ShipPeer) otherShip).getType();
                    otherBoundingBox.setRect(otherShip.getX()-type.getHalfWidth(), otherShip.getY()-type.getHalfHeight(), type.getWidth(), type.getHeight());
