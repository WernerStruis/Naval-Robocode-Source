...
public class ShipSnapshot extends RobotSnapshot implements IShipSnapshot{
	...
	public ShipSnapshot(ShipPeer shipPeer, boolean readoutText){
		...
		this.type = shipPeer.getType();
		IComponent[] componentArrayList = cp.getComponentsList();
		...
		for(IComponent component : componentArrayList){
			if(component instanceof CannonComponent){
				componentManager.add(new WeaponComponentSnapshot((CannonComponent)component));
			}
			...
			}else if(component instanceof MissileComponent){
				componentManager.add(new MissileComponentSnapshot((MissileComponent) component));
			}
		}
	...
	public void writeXml(XmlWriter writer, SerializableOptions options) throws IOException {
		writer.startElement(options.shortAttributes ? "s" : "ship"); {
			...
			writer.writeAttribute("type", getType().name());
			if (!options.skipNames) {
			...
			for(IComponentSnapshot ics : componentManager){
				BaseComponentSnapshot bcs = (BaseComponentSnapshot) ics;
				if(bcs instanceof WeaponComponentSnapshot){
					WeaponComponentSnapshot wps = (WeaponComponentSnapshot) bcs;
					wps.writeXml(writer, options);
				} else if(bcs instanceof RadarComponentSnapshot){
					RadarComponentSnapshot rcs = (RadarComponentSnapshot) bcs;
					rcs.writeXml(writer, options);
				} else if(bcs instanceof MineComponentSnapshot){
					MineComponentSnapshot mcs = (MineComponentSnapshot) bcs;
					mcs.writeXml(writer, options);
				}else if(bcs instanceof MissileComponentSnapshot){
					MissileComponentSnapshot mcs = (MissileComponentSnapshot) bcs;
					mcs.writeXml(writer, options);
				}

//				BaseComponentSnapshot bcs = (BaseComponentSnapshot) ics;
//				bcs.writeXml(writer, options);
			}
			writer.endElement();
