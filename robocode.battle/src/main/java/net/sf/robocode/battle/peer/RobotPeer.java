...
public /*final*/ class RobotPeer implements IRobotPeerBattle, IRobotPeer, ITransformable {
	...
	protected int lastExecutionTime = -1;
	...
	public void initializeRound(List<RobotPeer> robots, RobotSetup[] initialRobotSetups) {
		...
		if (!valid) {
				...
				if (isSentryRobot()) {
					...
					if (battle.countActiveSentries() > 0) {
						...
						if (HiddenAccess.getNaval()) {
							ShipType type = ShipType.getTypeByID(statics.getShipFlag());
			...
			ExecCommands newExecCommands = new ExecCommands(15.4, 1, ShipType.CORVETTE.getMaxSlots());
	...
	public void startRound(long waitMillis, int waitNanos) {
		...
		System.out.println("RobotPeer.java startRound() at: " + System.currentTimeMillis());
		System.out.println("--> Here a status is passed to the proxy");
		status.set(stat);
	...
	protected void fireBullets(List<BulletCommand> bulletCommands) {
		...
		for (BulletCommand bulletCmd : bulletCommands) {
			...
			newBullet = new BulletPeer(this, null, battleRules, bulletCmd.getBulletId());
	...
	public void updateBoundingBox() {
		ShipType type = ShipType.getTypeByID(statics.getShipFlag());
		boundingBox.setRect(x - type.getHalfWidth(), y - type.getProwOffset(), type.getWidth(), type.getHeight());
	}
...
//				double angle = atan2(dx, dy);
//				double dist = Math.hypot(dx, dy);
//				double[] slotBearings = new double[ShipType.getTypeByID(statics.getShipFlag()).getMaxSlots()];
//
//
//
//
			...
			if (statics.isTeamLeader()) {
				for (RobotPeer teammate : teamPeer) {
					if (teammate.isAlive() && teammate != this) {
						...
						BulletPeer sBullet = new BulletPeer(this, null, battleRules, 0);
