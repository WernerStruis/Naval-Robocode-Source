...
public class ScannedShipEventTest extends RobocodeTestBed{
	...
	private static final double predictedBearing = 94.57392125990089;
	...
	protected double getXMiddle(double x, double heading){
		return x + (ShipType.CARRIER.getProwOffset() * Math.cos(heading + Math.PI/2));
	}
	/**
	...
	protected double getYMiddle(double y, double heading){
		return y + (ShipType.CARRIER.getProwOffset() * Math.sin(heading + Math.PI/2));
	}
}
