...
public class WeaponComponentTest {
	...
	public void testTurnRadiansStopAtBlindSpot(String testName, double turnRemaining, boolean log){
		...
		while(!weaponComponent.getAtBlindSpot() && !Utils.isNear(degreesToTurn, 0.0)){
			...
			if(turnRight){
				degreesToTurn -= CannonType.SINGLE_BARREL.getTurnRate();
				if(degreesToTurn < 0)
					degreesToTurn = 0;
				if(weaponComponent.getAngle() + weaponComponent.getTurnRateRadians() > weaponComponent.getCopyOfBlindSpot().getFarRight())
					break;
			}
			else{
				degreesToTurn += CannonType.SINGLE_BARREL.getTurnRate();
				if(degreesToTurn > 0)
					degreesToTurn = 0;
				if(weaponComponent.getAngle() - weaponComponent.getTurnRateRadians() < Utils.normalRelativeAngle(weaponComponent.getCopyOfBlindSpot().getFarLeft()))
					break;
			}
