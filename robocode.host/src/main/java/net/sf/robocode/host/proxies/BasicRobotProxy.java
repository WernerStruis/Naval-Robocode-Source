...
public class BasicRobotProxy extends HostingRobotProxy implements IBasicRobotPeer {
	...
	public Bullet setFire(double power, int gunIndex) {
		setCall();
		return setFireImpl(power, gunIndex);
	}
	// asynchronous actions
	public Bullet setFire(double power) {
		setCall();
		return setFireImpl(power, NavalRules.IDX_WEAPON_FRONT);
	}
	...
	private final Bullet setFireImpl(double power, int gunIndex) {
	...
	private final Missile setLaunchImpl(double power) {
		...
		firedEnergy += power;
		firedHeat += NavalRules.getMissileHeat(power);
