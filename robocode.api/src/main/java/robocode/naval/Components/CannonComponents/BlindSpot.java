...
public class BlindSpot implements Serializable {
	...
	private boolean hasSecondBlindSpot;
	private BlindSpot secondBlindSpot;

	public BlindSpot getSecondBlindSpot() {
		return secondBlindSpot;
	}

	/**
	 * This is half the angle, in radians, of the blind arch.
	 * @see CannonComponent#BLIND_SPOT_ANGLE
	 * @see CannonComponent#HALF_BLIND_SPOT_ANGLE
	 */
	private double half;
	 ...
	 */
	public BlindSpot(double start, double extent, boolean hasSecondBlindSpot) {
		...
		if(hasSecondBlindSpot) {
			this.secondBlindSpot = new BlindSpot((PI) - Math.abs(start), extent, false);
		}
	...
	public BlindSpot(BlindSpot blindSpot){
		...
		this.secondBlindSpot = blindSpot.secondBlindSpot;
	}
	...
	public boolean inBlindSpot(double angle) {
		if (hasBlindSpot()) {
			...
			if(hasSecondBlindSpot && secondBlindSpot.inBlindSpot(angle)){
				return true;
			}
			return crossZero ?
	...
	private static class HiddenBlindSpotHelper implements ISerializableHelper, IHiddenComponentHelper {
		...
		public int sizeOf(RbSerializer serializer, Object object) {
			return 3 * RbSerializer.SIZEOF_DOUBLE + RbSerializer.SIZEOF_BOOL;
		}
		...
		public void serialize(RbSerializer serializer, ByteBuffer buffer, Object object) {
			...
			serializer.serialize(buffer, obj.hasSecondBlindSpot);
			serializer.serialize(buffer, obj.secondBlindSpot.getStart());
		}
		...
		public Object deserialize(RbSerializer serializer, ByteBuffer buffer) {
			...
			obj.hasSecondBlindSpot = serializer.deserializeBoolean(buffer);
			obj.secondBlindSpot = new BlindSpot(serializer.deserializeDouble(buffer), obj.end, false);
			return obj;
		}
	}
}
