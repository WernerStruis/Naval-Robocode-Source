...
public class AvailableRobotsPanel extends JPanel {
	...
	private final TransferRobotDialog transferRobotDialog;
	...
	public AvailableRobotsPanel(JButton actionButton, String title, JList actionList,
		...
		this.transferRobotDialog = null;
		initialize();
	}
	public AvailableRobotsPanel(String title, TransferRobotDialog transferRobotDialog) {
		super();
		this.title = title;
		this.actionButton = null;
		this.actionList = null;
		this.robotSelectionPanel = null;
		this.transferRobotDialog = transferRobotDialog;
		initialize();
	}
	...
	private void availableRobotsListSelectionChanged() {
		int sel = getAvailableRobotsList().getSelectedIndex();

		if (sel != -1) {
			if (actionList != null) {
				actionList.clearSelection();
			}
			IRobotSpecItem robotSpecification = ((ItemWrapper) getAvailableRobotsList().getModel().getElementAt(sel)).getItem();

			if (robotSelectionPanel != null) {
				robotSelectionPanel.showDescription(robotSpecification);
			}
			if(transferRobotDialog != null){
				transferRobotDialog.showDescription(robotSpecification);
			}
		} else {
			if (robotSelectionPanel != null) {
			...
			if(transferRobotDialog != null){
				transferRobotDialog.showDescription(null);
			}
		}
	}
	...
	private JList getAvailablePackagesList() {
		if (availablePackagesList == null) {
			...
			availablePackagesList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
