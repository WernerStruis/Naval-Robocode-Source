...
public class WindowManager implements IWindowManagerExt {
    ...
    private NewCompetitionDialog hostBattleDialog;
    private TransferRobotDialog transferDialog;
    ...
    public WindowManager(ISettingsManager settingsManager, IBattleManager battleManager, ICpuManager cpuManager, IRepositoryManager repositoryManager, IImageManager imageManager, IVersionManager versionManager, ICompetitionManager competitionManager) {
    ...
    public void showRobotTransfer() {
        if (transferDialog != null) {
            transferDialog.dispose();
            transferDialog = null;
        }

        transferDialog = net.sf.robocode.core.Container.factory.getComponent(TransferRobotDialog.class);
        transferDialog.setup(settingsManager);
        WindowUtil.packCenterShow(transferDialog);
    }
    ...
    public void showSplashScreen() {
        ...
        competitionManager.reload(versionManager.isLastRunVersionChanged());
    ...
    public void onRobotTransfered() {
        hostBattleDialog.refreshContestantList();
    }

    public void showNewCompetitionDialog(BattleProperties battleProperties) {
        try {
            battleManager.pauseBattle();

            manager.startCompetitionServer(battleProperties);

            hostBattleDialog = Container.createComponent(NewCompetitionDialog.class);
    ...
    public void onServerStarted(String ip, int port, BattleProperties properties){
        hostBattleDialog.setup(settingsManager, properties);
        hostBattleDialog.setAdress(ip, port);
        WindowUtil.packCenterShow(getRobocodeFrame(), hostBattleDialog);
    }

    public void sendRobot(String ipString, int portNumber, IRobotSpecItem robot) {
        manager.sendRobot(ipString, portNumber, robot);
    }

    public void closeCompetitionServer() {
        manager.closeCompetitionServer();
    }
