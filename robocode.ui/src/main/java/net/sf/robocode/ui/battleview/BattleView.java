...
import net.sf.robocode.battle.snapshot.components.*;
...
public class BattleView extends Canvas {
    ...
    public void paint(Graphics g) {
        ...
        if (lastSnapshot != null) {
            ...
            if(ExternalManager.cb != null) {
//                System.out.println("BATTLEVIEW.JAVA :162 - CLOSING SOCKET");
                ExternalManager.cb.onBattleEnded();
            }else {
    ...
    private void update(ITurnSnapshot snapshot) {
        if(ExternalManager.cb != null){
            //TODO flip condition to != null
            //TODO send to ExternalManager
            ExternalManager.cb.onIncomingSnapshot(snapshot);
        }else {
            if (!initialized) {
                initialize();
            }
...
//        System.out.println("BATTLEVIEW.JAVA :153 - INITIALIZING (SET UP SOCKET?)");
    ...
    private void drawRealShips(Graphics2D g, ITurnSnapshot snapShot) {
        ...
        IMissileSnapshot[] missileSnapshots = snapShot.getMissiles();
        ...
        for (IShipSnapshot shipSnapshot : shipSnapshots) {
            if (shipSnapshot.getState().isAlive()) {
                ...
                double pivotX = x;
                double pivotY = y - shipSnapshot.getType().getProwOffset();

                // Body
                at = Coordinates.getAffineTransformShip(shipSnapshot);
                at.setToIdentity();
                at.translate(0, ShipType.getTypeByID(shipSnapshot.getShipFlag()).getProwOffset());
                at.rotate(shipSnapshot.getBodyHeading(), pivotX, pivotY);
                at.translate(x, y);

                RenderImage robotRenderImage = imageManager.getColoredBodyRenderNavalImage(shipSnapshot.getBodyColor(), shipSnapshot.getType());
                robotRenderImage.setTransform(at);
                robotRenderImage.paint(g);
                ...
                drawShipBoundingBox(g, shipSnapshot);
            }
        }
        for (IMissileSnapshot missileSnapshot : missileSnapshots) {
            drawMissileBoundingBox(g, missileSnapshot);
        }
    }

    private void drawMissileBoundingBox(Graphics2D g, IMissileSnapshot snapShot) {
        if (drawBoundingBoxes) {
            g.setColor(new Color(0, 180, 6, 150));
            g.drawOval((int) snapShot.getX() - NavalRules.HALF_MISSILE_WIDTH_OFFSET, (int) snapShot.getY() - NavalRules.HALF_MISSILE_HEIGHT_OFFSET, NavalRules.MISSILE_WIDTH, NavalRules.MISSILE_HEIGHT);
        }
    ...
    private void drawShipBoundingBox(Graphics2D g, IShipSnapshot snapShot) {
        if (drawBoundingBoxes) {
            g.setColor(new Color(15, 0, 180, 150));

            Rectangle2D shipBox = snapShot.getBoundingBox();
            AffineTransform at2 = new AffineTransform();
            at2.translate(snapShot.getX() - shipBox.getWidth() / 2, snapShot.getY() - snapShot.getType().getProwOffset());
            at2.rotate(snapShot.getBodyHeading(), snapShot.getType().getHalfWidth(), snapShot.getType().getProwOffset());

            Polygon poly1 = Collision.getPolygon(shipBox, at2);
    ...
    private void drawNavalBlindSpots(Graphics2D g, IShipSnapshot snapShot) {
        if (drawNavalBlindSpots) {
            ...
            double arcRadius = 65;
            double arcOffset = arcRadius / 2;

            ArrayList<IComponentSnapshot> cm = snapShot.getComponents();
            ShipType st = snapShot.getType();
            for (IComponentSnapshot cs : cm) {
                g.setColor(new Color(180, 50, 80, 150));

                if (cs instanceof WeaponComponentSnapshot) {
                    double slotOffset = (ComponentManager.getSlotOffset(((WeaponComponentSnapshot) cs).getSlotIndex(), st));
                    if (((WeaponComponentSnapshot) cs).getSlotIndex() != 0 && ((WeaponComponentSnapshot) cs).getSlotIndex() != (st.getMaxSlots() - 1)) {
                        //our weapon is in the middle
                        g.setColor(new Color(3, 180, 0, 150));

                        BlindSpotSnapshot bs = ((WeaponComponentSnapshot) cs).getBlindSpot();
                        Arc2D.Double arc = new Arc2D.Double();
                        arc.setArc(snapShot.getX() - arcOffset + (slotOffset * Math.cos(bodyHeadingRadians + Math.PI / 2)),
                                snapShot.getY() - arcOffset + (slotOffset * Math.sin(bodyHeadingRadians + Math.PI / 2)),
                                arcRadius,
                                arcRadius,
                        ...
                        Arc2D.Double secondarc = new Arc2D.Double();
                        secondarc.setArc(snapShot.getX() - arcOffset + (slotOffset * Math.cos(bodyHeadingRadians + Math.PI / 2)),
                                snapShot.getY() - arcOffset + (slotOffset * Math.sin(bodyHeadingRadians + Math.PI / 2)),
                                arcRadius,
                                arcRadius,
                                Math.toDegrees(Math.PI / 2 - bodyHeadingRadians + bs.getSecondstart()),
                                Math.toDegrees(bs.getExtent()),
                                Arc2D.PIE);
                        g.fill(secondarc);
                    } else {
                        //our weapon is on the back
                        BlindSpotSnapshot bs = ((WeaponComponentSnapshot) cs).getBlindSpot();
                        Arc2D.Double arc = new Arc2D.Double();
                        arc.setArc(snapShot.getX() - arcOffset + (slotOffset * Math.cos(bodyHeadingRadians + Math.PI / 2)),
                                snapShot.getY() - arcOffset + (slotOffset * Math.sin(bodyHeadingRadians + Math.PI / 2)),
...
//                Arc2D.Double arc = new Arc2D.Double();
//                arc.setArc(snapShot.getX() - 50 + (st.getfWeaponOffset() * Math.cos(bodyHeadingRadians + Math.PI / 2)),
//                        snapShot.getY() - 50 + (st.getfWeaponOffset() * Math.sin(bodyHeadingRadians + Math.PI / 2)),
//                        100,
//                        100,
//                        Math.toDegrees(Math.PI / 2 - bodyHeadingRadians + bs.getStart()),
//                        Math.toDegrees(bs.getExtent()),
...
//                g.fill(arc);
//            }
//            if (NavalRules.IDX_WEAPON_BACK < cm.size()) {
//                component = cm.get(NavalRules.IDX_WEAPON_BACK);
//                if (component instanceof WeaponComponentSnapshot) {
//                    WeaponComponentSnapshot weapon = (WeaponComponentSnapshot) component;
//                    BlindSpotSnapshot bs = weapon.getBlindSpot();
//                    Arc2D.Double arc = new Arc2D.Double();
//                    arc.setArc(snapShot.getX() - 50 + (st.getbWeaponOffset() * Math.cos(bodyHeadingRadians + Math.PI / 2)),
//                            snapShot.getY() - 50 + (st.getbWeaponOffset() * Math.sin(bodyHeadingRadians + Math.PI / 2)),
//                            100,
//                            100,
//                            Math.toDegrees(Math.PI / 2 - bodyHeadingRadians + bs.getStart()),
//                            Math.toDegrees(bs.getExtent()),
...
//            }
    ...
    private void drawBoundingBoxShip(Graphics2D g, IRobotSnapshot robotSnapshot) {
        AffineTransform at2 = new AffineTransform();
        ShipType st = ((IShipSnapshot) robotSnapshot).getType();
        Rectangle2D rect2 = new Rectangle(0, 0, st.getWidth(), st.getHeight());

        at2.translate(robotSnapshot.getX() - st.getHalfWidth(), robotSnapshot.getY() - st.getProwOffset());
        at2.rotate(robotSnapshot.getBodyHeading(), st.getHalfWidth(), st.getProwOffset());

        Polygon poly1 = Collision.getPolygon(rect2, at2);

        g.setColor(Color.MAGENTA);
        g.draw(poly1);
        g.fillOval((int) (robotSnapshot.getX() + (st.getProwOffset() * Math.cos(robotSnapshot.getBodyHeading() + Math.PI / 2)) - 5),
                (int) (robotSnapshot.getY() + (st.getProwOffset() * Math.sin(robotSnapshot.getBodyHeading() + Math.PI / 2)) - 5),
                10, 10);
    }
